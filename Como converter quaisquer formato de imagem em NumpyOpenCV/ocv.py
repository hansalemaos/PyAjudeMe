import base64
from PIL import Image
from a_cv_imwrite_imread_plus import (
open_image_in_cv
)
base64img2 = r""
pininterestlogo = "https://camo.githubusercontent."
byteimage = base64.b64decode(base64img2)
pilimage = Image.open(r"F:\00401.jpg")
filepath = r"F:\00401.jpg"

base64img2 = open_image_in_cv(base64img2,
                              channels_in_output=3)
pininterestlogo = open_image_in_cv(pininterestlogo)
byteimage = open_image_in_cv(byteimage)
pilimage = open_image_in_cv(pilimage)
filepath = open_image_in_cv(filepath)

print(f'{base64img2=}')
print(f'{pininterestlogo=}')
print(f'{byteimage=}')
print(f'{pilimage=}')
print(f'{filepath=}')